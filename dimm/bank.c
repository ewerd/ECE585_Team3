/**
 * @file bank.c 
 * @brief Bank Level Memory Controller ADT source code file
 *
 * @detail 	This is the source code file which contains the bank module level functionality.
 * 		It is the lowest level of the memory control simulation and is queried recursively
 * 		to determine the timing constraints for various memory commands.
...
uint8_t bank_recoveryTime(memCmd_t firstOp, memCmd_t secOp)
{
	...
	switch(firstOp)
	{
		...
		case PRECHARGE:
		if (secOp == ACTIVATE)
			return B_PRECHARGE_TO_ACTIVATE;
		if (secOp == READ || secOp == WRITE)
			return B_PRECHARGE_TO_RW;
		if (secOp == PRECHARGE)
			return B_PRECHARGE_TO_PRECHARGE;
		case ACTIVATE:
		if (secOp == PRECHARGE)
			return B_ACTIVATE_TO_PRECHARGE;
		if (secOp == ACTIVATE)
			return B_ACTIVATE_TO_ACTIVATE;
		if (secOp == READ || secOp == WRITE)
			return B_ACTIVATE_TO_RW;
		case READ:
		if (secOp == PRECHARGE)
			return B_READ_TO_PRECHARGE;
		if (secOp == ACTIVATE)
			return B_READ_TO_ACTIVATE;
		if (secOp == READ)
			return B_READ_TO_READ;
		if (secOp == WRITE)
			return B_READ_TO_WRITE;
		case WRITE:
		if (secOp == PRECHARGE)
			return B_WRITE_TO_PRECHARGE;
		if (secOp == ACTIVATE)
			return B_WRITE_TO_ACTIVATE;
		if (secOp == READ)
			return B_WRITE_TO_READ;
		if (secOp == WRITE)
			return B_WRITE_TO_WRITE;
	}
	Fprintf(stderr, "Warning in bank.bank_recoveryTime(): Missing case for commands\n");
	return 0;
}
...
int bank_precharge(bank_t *bank, unsigned long long currentTime)
{
	...
	bank->nextActivate = currentTime + B_PRECHARGE_TO_ACTIVATE; // next activate time is when bank is finished precharging (TRP = time for precharge)
	return B_PRECHARGE_TO_ACTIVATE; // return time it will take to precharge bank (TRP)
}
